...
const MODULATE_NORMAL = Color(0.75, 0.75, 0.75, 0.25)
...
@export var progress_for_stage: ProgressForStage:
	...
	set(value):
		...
		%TextureRectBronze.modulate = MODULATE_EMPHASIS if value.medals > 0 else MODULATE_NORMAL
		%TextureRectSilver.modulate = MODULATE_EMPHASIS if value.medals > 1 else MODULATE_NORMAL
		%TextureRectGold.modulate = MODULATE_EMPHASIS if value.medals > 2 else MODULATE_NORMAL
